...
def main():
    ...
    with col1:
        ...
        if st.button('信箱 : leejs@fcu.edu.tw', use_container_width=True):
            text_to_be_copied = 'leejs@fcu.edu.tw'
            pyperclip.copy(text_to_be_copied)

        if st.button('分機 : #3700 #3767', use_container_width=True):
            text_to_be_copied = '0424517250#3700 #3767'
            pyperclip.copy(text_to_be_copied)
    ...
    with col2:
        st.image('./assets/imgs/dbs-test-image.jpg')
    ...
    with fast_link_col2:
        ...
        # streamlit expander for streamlit map
